...
void stepMotor_step(void)
{
   ...
   */
  MultiWordSub(&sigInput.timeStamp.chunks[0U],
               &stepMotor_DW.UnitDelay_DSTATE.chunks[0U], &rtb_Subtract.chunks
               [0U], 2);
  ...
  if (rtb_LogicalOperator1_kl) {
    rtb_UnitDelay_j = (uMultiWord2Double(&rtb_Subtract.chunks[0U], 2, 0) <
                       stepMotor_ConstB.Gain);
  } else {
  ...
  if (sigInput.steps > 0U) {
    rtb_Switch1_n = sigInput.steps;
  } else {
    rtb_Switch1_n = (uint32_T)rtb_Abs;
  }
   ...
   *  UnitDelay: '<S2>/Unit Delay1'
   */
  rtb_LogicalOperator = (stepMotor_DW.UnitDelay1_DSTATE_f && (rtb_Switch1_n > 0U)
    && (!stepMotor_DW.UnitDelay_DSTATE_ip));
   ...
   */
  MultiWordSub(&sigInput.timeStamp.chunks[0U],
               &stepMotor_DW.UnitDelay1_DSTATE.chunks[0U], &rtb_dt.chunks[0U], 2);
  ...
  if (rtb_FixPtRelationalOperator_j) {
     ...
     *  Sum: '<S2>/Subtract'
     */
    if (rtb_Switch1_n > 200U) {
      rtb_Switch_k = look1_iflf_binlcapw((real32_T)((real_T)
        stepMotor_DW.Counter_Count / ((real_T)rtb_Switch1_n - 1.0)) * 100.0F,
    ...
    stepMotor_DW.UnitDelay_DSTATE_i = 29.8F * rtb_Switch_k;
  }
   ...
   *  UnitDelay: '<S2>/Unit Delay'
   */
  stepMotor_DW.UnitDelay_DSTATE_f = (uMultiWordSingle(&rtb_dt.chunks[0U], 2, 0) <
    0.5F * stepMotor_DW.UnitDelay_DSTATE_i);
   ...
   *  UnitDelay: '<S12>/Unit Delay'
   */
  stepMotor_DW.UnitDelay_DSTATE_ip = (uMultiWordSingle(&rtb_dt.chunks[0U], 2, 0)
    < stepMotor_DW.UnitDelay_DSTATE_i);

  /* RelationalOperator: '<S5>/Relational Operator1' incorporates:
   *  Constant: '<S5>/Constant3'
   */
  rtb_LogicalOperator1 = (uMultiWord2Double(&rtb_Subtract.chunks[0U], 2, 0) >=
    298.0);
  ...
  if (((int32_T)rtb_LogicalOperator1 > (int32_T)
    ...
    stepMotor_DW.UnitDelay_DSTATE = sigInput.timeStamp;
  }
